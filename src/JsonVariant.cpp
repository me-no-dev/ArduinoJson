...
size_t JsonVariant::writeTo(JsonWriter &writer) const {
  if (_type == JSON_ARRAY)
    return _content.asArray->writeTo(writer);

  else if (_type == JSON_OBJECT)
    return _content.asObject->writeTo(writer);

  else if (_type == JSON_STRING)
    return writer.writeString(_content.asString);

  else if (_type == JSON_UNPARSED)
    return writer.writeRaw(_content.asString);

  else if (_type == JSON_INTEGER)
    return writer.writeInteger(_content.asInteger);

  else if (_type == JSON_BOOLEAN)
    return writer.writeBoolean(_content.asInteger != 0);

  else if (_type >= JSON_FLOAT_0_DECIMALS) {
    uint8_t decimals = static_cast<uint8_t>(_type - JSON_FLOAT_0_DECIMALS);
    return writer.writeFloat(_content.asFloat, decimals);
  }
  return 0;
}
}
